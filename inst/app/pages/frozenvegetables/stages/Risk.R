...
fv_Risk_server <- function(input, output, session, suffix, datCook) {
  ...
  datRisk <- eventReactive(input$updateFV, {
    # Generate data and store it in reactive values if NULL
    if (is.null(values$data)) {
      progress <- shiny::Progress$new()
      on.exit(progress$close())
      progress$set(message = "Risk", value = 8/8)
      
      values$data <- fvGenerate_datRisk(input, prefix, datCook)
      # Need a function in the following functions
      datFn <- function() values$data
      if (isTRUE(getOption("myVerbose"))) print("reevaluate stat Risk")
      riskStatsServer("fv_risk_stats",  data = datFn)
      riskDistServer("fv_risk_dist",    data = datFn)
    }
  ...
  observe({
    ...
    inputListFVRisk <- c("Model", "PopulationJEMRA", "PopulationPouillot",
                         "PopulationEFSA", "PopulationEFSALV", "PopulationEFSAV",
                         "PopulationEFSAMV", "PopulationFritsch")
    lapply(inputListFVRisk, function(x) input[[paste0(prefix,x)]])
...
fvGenerate_datRisk <- function(input, prefix, datCook) {
  if (isTRUE(getOption("myVerbose"))) print("reevaluate Risk")
