...
fv_Production_server <- function(input, output, session, suffix) {
  ...
  datProd <- eventReactive(input[["updateFV"]], {
    ...
    if (is.null(values$data)) {
      ...
      values$data <- generate_datProd(input, prefix, session)
  ...
  observe({
    ...
    inputListFVProd <- c("seed", "n_lots", "size_lot", "unit_size",
...
generate_datProd <- function(input, prefix, session) { #first stage, no input
  ...
  if (is_shinyapps && (nLots > 500 || sizeLot > 500)) {
    showModal(modalDialog(
      ...
      HTML("Due to storage limitations on this server, the Frozen Vegetables simulation is restricted to
            a maximum of 500 lots and 500 units per lot. For unrestricted explorations install the application locally:
    ...
    if (nLots > 500) {
      updateNumericInput(session, paste0(prefix, "n_lots"), value = 500)
      validate(need(FALSE, "Simulation size exceeded"))
    }

    if (sizeLot > 500) {
      updateNumericInput(session, paste0(prefix, "size_lot"), value = 500)
      validate(need(FALSE, "Simulation size exceeded"))
    }
  }
...
fv_ProductionInputs_ui <- function(id) {  
  ...
  div(  
    ...
    sliderInput(ns("log_sd_c"), 
                label = makeHelp("St. dev. of Counts (log10 CFU/g) (<i>C0SdLog</i>)", 'Lot2LotGen'),
