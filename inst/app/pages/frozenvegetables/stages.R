...
fv_stages_ui <- function(id, fv_stages) {
 # Initialize namespace
  ...
  button <- div(class="centered", 
                fluidRow(
                  column(6, actionButton("updateFV", HTML("<b>Update Results</b>"), width="100%", icon = icon("rotate"))),
                  column(6, actionButton("resetFV", HTML("<b>Reset Parameters</b>"), width="100%", icon = icon("undo")))
                )
              )
...
#    h3("Variability of contamination"),
  ...
  # fluidRow(column(8, actionButton("update", HTML("<b>Update Results</b>"), width="100%", icon = icon("rotate"))))
