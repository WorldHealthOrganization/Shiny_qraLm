...
sf_Risk_server <- function(input, output, session, suffix, datPort) {
  ...
  datRisk <- eventReactive(input$updateSF, {
    ...
    if (is.null(values$data)) {
      ...
      values$data <- sfGenerate_datRisk(input, prefix, datPort)
      ...
      riskStatsServer("sf_risk_stats",  data = datFn)
      riskDistServer("sf_risk_dist",    data = datFn)
    }
  ...
  observe({
    ...
    inputListSFRisk <- c("Model", "PopulationJEMRA", "PopulationPouillot",
                         "PopulationEFSA", "PopulationEFSALV", "PopulationEFSAV",
                         "PopulationEFSAMV", "PopulationFritsch")
    lapply(inputListSFRisk , function(x) input[[paste0(prefix,x)]])
...
sfGenerate_datRisk <- function(input, prefix, datCook) {
