...
sf_stages_ui <- function(id, sf_stages) {
  ...
  button <- div(class="centered", 
                fluidRow(
                  column(6, actionButton("updateSF", HTML("<b>Update Results</b>"), width="100%", icon = icon("rotate"))),
                  column(6, actionButton("resetSF", HTML("<b>Reset Parameters</b>"), width="100%", icon = icon("undo")))
                )
              )
